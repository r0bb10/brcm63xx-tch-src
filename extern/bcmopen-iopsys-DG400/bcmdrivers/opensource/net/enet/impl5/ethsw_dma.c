        ...
        if ((portmap & (1U<<i)) != 0) {
            ...
            while(phyinit->u.op.op != BP_MDIO_INIT_OP_NULL)
            {
                if(phyinit->u.op.op == BP_MDIO_INIT_OP_WRITE){
                    data=(uint16)phyinit->u.write.data;
                    ethsw_phy_wreg(phy_id, phyinit->u.write.reg, &data);
    ...
    for (i = 0; i < NUM_RGMII_PORTS; i++) {
        {
            ...
            if ((IsRGMII(phyid) && IsPhyConnected(phyid)) ||
                ...
                /* phy 54210 is init using board parameters
                   problem with that is that it is per board so it needs to be
                   configured per board. leave this stub here for future change.
                */
#if 0
                if ((phy_identifier & BCM_PHYID_M) == (BCM54210_PHYID2 & BCM_PHYID_M)) {

                    /*disable RDB mode */
                    v16 = 0x0087;
                    ethsw_phy_wreg(phyid, 0x1E, &v16);
                    v16 = 0x8000;
                    ethsw_phy_wreg(phyid, 0x1F, &v16);
...
#if 0
                    /* Turn off super-isolate mode */
                    v16 = 0x2007;
                    ethsw_phy_wreg(phyid, MII_REGISTER_18, &v16);
                    v16 = 0x06c2;
                    ethsw_phy_wreg(phyid, MII_REGISTER_18, &v16);
#endif
                    printk("BCM54210:%s %x %x\n",__func__,phyid,v16);
                }
#endif
				if (phy_identifier == AR8035_PHYID2) {
                    v16 = 0xb100;
