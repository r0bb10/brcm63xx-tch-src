...
typedef enum {
    ...
    rdpa_epon_attr_link_mpcp_state = 10, /* link_mpcp_state : RF : enum/4[8] : Get the link MPCP regitration state */
    rdpa_epon_attr_mcast_link = 11, /* mcast_link : RWF : aggregate/5[8] epon_mcast_link(rdpa_epon_mcast_link_t) : EPON multicast link properties */
    rdpa_epon_attr_fec_enable = 12, /* fec_enable : RWF : aggregate/2[8] epon_fec_enable(rdpa_epon_fec_enable_t) : enable EPON MAC FEC for us/ds or both */
    rdpa_epon_attr_burst_cap_map = 13, /* burst_cap_map : RWF : aggregate/16[8] burst_cap_per_priority(rdpa_epon_burst_cap_per_priority_t) : all links burst cap per pri */
