...
static bdmf_index ic_get_ingress_port(rdpactl_classification_rule_t *rule)
{
    ...
    if (rule->ingress_port_id == RDPACTL_IF_LAN4)
       return rdpa_if_lan4;

    if (rule->ingress_port_id == RDPACTL_IF_LAN5)
       return rdpa_if_lan5;

    if (rule->ingress_port_id == RDPACTL_IF_LAN6)
       return rdpa_if_lan6;

    if (rule->ingress_port_id == RDPACTL_IF_LAN7)
       return rdpa_if_lan7; 
