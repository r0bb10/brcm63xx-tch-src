...
static int __init brcm_board_init( void )
{
    ...
    board_util_init();
#if defined(CONFIG_BCM_6802_MoCA)
    board_mocaInit(); // Must be called before board_extirq_init()
#endif
    board_extirq_init();
    board_wl_init();
...
EXPORT_SYMBOL(inteno_netlink_linkscan);
EXPORT_SYMBOL(inteno_netlink_dsl);
EXPORT_SYMBOL(inteno_netlink_string);
