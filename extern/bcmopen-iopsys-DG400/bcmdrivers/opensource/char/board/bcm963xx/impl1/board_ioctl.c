                       ...
                       unsigned int command, unsigned long arg )
{
    ...
    case BOARD_IOCTL_FTTDP_DSP_BOOTER:    
        ...
        break;
#endif
    case BOARD_IOCTL_NVRAM_WRITE:
        if (copy_from_user((void*) &ctrlParms, (void*) arg, sizeof(ctrlParms)) == 0) {
            ret = updateDefaultNvram();
    ...
    case BOARD_IOCTL_SET_SES_LED:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
			...
			else if(ctrlParms.strLen == 2 || ctrlParms.strLen == 9 || ctrlParms.strLen == 20)
            {
            	inteno_wps_msg_handler(ctrlParms.strLen,ctrlParms.string);
                ctrlParms.result = 0;
                __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
                ret = 0;
            }
    ...
    case BOARD_IOCTL_GET_GPIO:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            kerSysSetGpioDirInput(ctrlParms.strLen);
    ...
    case BOARD_IOCTL_SPI_INIT:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
                /* mode is coded in the string pointer as a number */
                int mode = (int)ctrlParms.string; 

                ctrlParms.result = BcmSpiReserveSlave2(HS_SPI_BUS_NUM,
                                                       ctrlParms.action,
                                                       ctrlParms.offset,
                                                       mode,
	...
	case BOARD_IOCTL_SPI_WRITE:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
	    ctrlParms.result = BcmSpiSyncTrans((unsigned char *) ctrlParms.string, NULL, 0, ctrlParms.strLen, HS_SPI_BUS_NUM, ctrlParms.action);
            __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
            ret = 0;
        }
        else {
    ...
    case BOARD_IOCTL_BOOT_IMAGE_OPERATION:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case BOOT_GET_IMAGE_VERSION:
                ...
                ctrlParms.result = kerSysGetImageVersion((int) ctrlParms.action,
	...
	case BOARD_IOCTL_GET_SERIAL_NUMBER:
				...
				if (copy_from_user((void *) &ctrlParms, (void *) arg, sizeof(ctrlParms)) == 0)
				{
					...
					if (NULL == (pNvramData = readNvramData())) {
						printk("BOARD_IOCTL_GET_SERIAL_NUMBER: could not read nvram data\n");
						break;
					}
					memcpy(ctrlParms.string, &pNvramData->szSerialNumber, strlen(pNvramData->szSerialNumber));
					ctrlParms.strLen = strlen(pNvramData->szSerialNumber);
					ctrlParms.result = 0;
					__copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms,
						sizeof(BOARD_IOCTL_PARMS));
					ret = 0;
    ...
    case BOARD_IOCTL_GETSET_BOOT_INACTIVE_IMAGE:
        ret = -EFAULT;
#if defined(CONFIG_BCM96858) || defined(CONFIG_BCM96848) || defined(CONFIG_BCM96838)
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
