...
static Bool sesBtn_pressed(void)
{
    ...
    if( sesBtn_polling == 0 )
    {
#if defined(CONFIG_BCM96838) || defined(CONFIG_BCM94908) || (CONFIG_BCM_CHIP_REV == 0x6858A0)
        if ((sesBtn_irq >= INTERRUPT_ID_EXTERNAL_0) && (sesBtn_irq <= INTERRUPT_ID_EXTERNAL_5)) {
#elif defined(CONFIG_BCM96836) || defined(CONFIG_BCM963158) || (defined(CONFIG_BCM96858) && (CONFIG_BCM_CHIP_REV != 0x6858A0))
...
#elif defined(CONFIG_BCM96858) || defined(CONFIG_BCM94908) || defined(CONFIG_BCM96836) || defined(CONFIG_BCM963158)
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM94908) || defined(CONFIG_BCM96858) || defined(CONFIG_BCM96836) || defined(CONFIG_BCM963158)
...
#define WPS_MSG_STATUS        2
#define WPS_MSG_LED           9
#define WPS_MSG_STA           20
static void wps_led_message_handler(char led_type,char * wl_name)
{
	unsigned char msg[SEND_MSG_LEN];
	char * name_buf = wl_name;
	int index = (int)led_type;
	
	memset(msg,0,SEND_MSG_LEN);
	
	if(led_type > kLedStateUserWpsSessionOverLap || led_type < 0)
		return;
		
	if(led_st_str[index] != NULL)
	{
		snprintf(msg,SEND_MSG_LEN,WPS_LED_MSG_STR,led_st_str[index],name_buf);
		inteno_netlink_string(msg);
	}
}

void inteno_wps_msg_handler(int cmd_len, char * msg)
{
	switch(cmd_len)
	{
		case WPS_MSG_STATUS:
		...
		case WPS_MSG_LED:
			{
				wps_led_message_handler(msg[0],&msg[1]);
			}
			break;
		case WPS_MSG_STA:
			break;
...
void sesLed_ctrl(int action)
{
    ...
    if(sesLed_gpio == BP_NOT_DEFINED)
        return;
	//wps_led_message_handler(blinktype);
