...
int enet_ioctl_ethsw_info(struct net_device *dev, struct ethswctl_data *e, BcmEnet_devctrl *priv)
{
    ...
    /* Technicolot added start for lan/wan switch feature */
    /* only check switch port unit=1, and only when LanWanMux is configured to WAN */
    if((e->val == 1) && (LanWanMux == 0) && (BpGetLanWanMuxSwPort(e->val, &lanWanMuxPort) == BP_SUCCESS)) {
        BCM_ENET_DEBUG("enet_ioctl_ethsw_info:got lan/wan mux port,lanWanMuxPort=%d,old port_map=0x%x\n", lanWanMuxPort, port_map);
        port_map &= ~(1 << lanWanMuxPort);
    }
    /* Technicolot added end */
    phy_map = port_map;
