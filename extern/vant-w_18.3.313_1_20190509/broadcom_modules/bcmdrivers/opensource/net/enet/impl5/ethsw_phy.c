...
u8 firmware_8488x[] =
#include "BCM8488-A0-v00-00-10-TCM.hex"
;
...
enum {SFP_STATUS_UNPLUG, SFP_STATUS_PLUGIN, SFP_STATUS_LINK_DOWN, SFP_STATUS_LINK_UP};
...
static int sfp_hotplug_create_event(int sfp_status_event)
{
    ...
    switch(sfp_status_event)
    {
            case SFP_STATUS_UNPLUG:
                 event->action = "unplug";
                 break;
            case SFP_STATUS_PLUGIN:
                 event->action = "plugin";
                 break;
            case SFP_STATUS_LINK_DOWN:
                 event->action = "linkdown";
                 break;
            case SFP_STATUS_LINK_UP:
                 event->action = "linkup";
                 break;
            default:
                 printk("Wrong status event for Sfp module\n");
                 break;
    }
...
int pre_sfp_status = SFP_MODULE_OUT;
...
static int proc_get_sfp_status(struct seq_file *seq, void *v)
{
    if(sfp_status == SFP_MODULE_OUT)
        seq_printf(seq, "SFP Module Status:unplug\n");
    else if(sfp_status == SFP_MODULE_IN)
        seq_printf(seq, "SFP Module Status:plug in\n");
    else if(sfp_status == SFP_LINK_UP)
        seq_printf(seq, "SFP Module Status:link up\n");
    else
        seq_printf(seq, "SFP Module Status:unkown\n");

    return 0;
}

#if defined(CONFIG_4x2_CROSSBAR_SUPPORT) || defined(CONFIG_5x3_CROSSBAR_SUPPORT) || defined(CONFIG_3x2_CROSSBAR_SUPPORT)
static int proc_get_crossbar_status(struct seq_file *seq, void *v)
{
...
static int proc_open_sfp_status(struct inode *inode, struct file *file)
{
    return single_open(file, proc_get_sfp_status, NULL);
}

static int proc_open_crossbar_status(struct inode *inode, struct file *file)
{
    return single_open(file, proc_get_crossbar_status, NULL);
}

static struct file_operations sfp_proc_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_open_sfp_status,
    .read       = seq_read,
    .release    = single_release,
};

static struct file_operations crossbar_proc_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_open_crossbar_status,
    .read       = seq_read,
    .release    = single_release,
};
...
void ethsw_init_serdes(void)
{
    ...
    proc_create_data(SFP_STATUS_PROC_NAME, 0644, NULL, &sfp_proc_fops, NULL);
    proc_create_data(CROSSBAR_STATUS_PROC_NAME, 0644, NULL, &crossbar_proc_fops, NULL);
...
#define I2CInitDetectDelay 8
    ...
    if ( ethsw_sfp_module_detected() == 0)
    {
        if(sfp_module_type != SFP_NO_MODULE)
        {
            ...
            sfp_hotplug_create_event(SFP_STATUS_UNPLUG);
    ...
    if (sfp_module_type == SFP_NO_MODULE)
    {
...
#endif
        {
            ...
            sfp_hotplug_create_event(SFP_STATUS_PLUGIN);
#if defined(CONFIG_I2C)
            i2cDetectDelay = 0;
...
void ethsw_sfp_init(int phyId)
{
    ...
    {
        /* NG-104212:work round for DGA4132, DGA4131's SFP present signal is connected to GPIO 114,
         * while the default state of GPIO 114 after CPU reset is output high level, but for the
         * Hisense LTE3415 SFP module, the SFP present signal is set to low level only once after SFP is plugged in,
         * so when board warm restart, the SFP present will be pull high by CPU, and it remains at high level later,
         * this will lead to SFP module can't be detected after warm restart. The work round soultion is to set the
         * gpio 114 to output and set it to low level for 1s, then set GPIO 114 back to input again. This works for
         * the GPIO 114 is pull high a outside register to 3.3V, so if there is no SFP plugged in, after GPIO 114
         * is set to input, the GPIO value will recover to high level again. If there is SFP module plugged in, GPIO 114
         * will stay at low level after change the dir to input. */
         if(sfpDetectionGpio == BP_GPIO_114_AH) {
             printk("GPIO 114's current dir is %s(0x%x),value=0x%x\n",
                   (GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))) ? "out" : "in",
                   (unsigned int)(GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))),
                   (unsigned int)(GPIO->GPIOio[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))));
             kerSysSetGpioDir(BP_GPIO_114_AH);
             kerSysSetGpioState(BP_GPIO_114_AH, kGpioInactive);
             mdelay(1000);
             printk("GPIO 114's new dir is %s(0x%x),value=0x%x,mdelay 1s\n",
                   (GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))) ? "out" : "in",
                   (unsigned int)(GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))),
                   (unsigned int)(GPIO->GPIOio[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))));
        }
        ...
        if(sfpDetectionGpio == BP_GPIO_114_AH) {
            printk("GPIO 114's last dir is %s(0x%x),value=0x%x\n",
                  (GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))) ? "out" : "in",
                  (unsigned int)(GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))),
                  (unsigned int)(GPIO->GPIOio[GPIO_NUM_TO_ARRAY_IDX(BP_GPIO_114_AH)] & (GPIO_NUM_TO_MASK(BP_GPIO_114_AH))));
        }
...
sfp_module_in:
            ...
            sfp_status = SFP_MODULE_IN;
            if (pre_sfp_status == SFP_LINK_UP)
                sfp_hotplug_create_event(SFP_STATUS_LINK_DOWN);
            pre_sfp_status = SFP_MODULE_IN;
            goto sfp_end;
...
sfp_link_up:
            ...
            if (sfp_status != pre_sfp_status)
                sfp_hotplug_create_event(SFP_STATUS_LINK_UP);
            pre_sfp_status = SFP_LINK_UP;
            goto sfp_end;
    }
