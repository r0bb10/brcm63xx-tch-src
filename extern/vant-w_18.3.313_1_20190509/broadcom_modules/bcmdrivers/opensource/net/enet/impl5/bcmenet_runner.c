    ...
    for (logPort = 0; port_map; port_map >>= 1, logPort++)
    {
        ...
        else
        {
...
#endif
            {
                ...
                /* detect the SFP module status even when non Serdes PHY is up */
                {
                    int cb_port;
                    int phyId;
                            ...
                            cb_port = enet_get_next_crossbar_port(logPort, cb_port)) 
                    {
                        ...
                        if( cb_port == muxExPort || !IsSerdes(phyId) ) continue;
                        ethsw_phy_stat(unit, physical_port, cb_port);
                    }
