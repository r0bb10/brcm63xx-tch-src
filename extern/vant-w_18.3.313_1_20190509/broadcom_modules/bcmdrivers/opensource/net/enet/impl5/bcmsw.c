...
void extsw_init_config(void)
{
...
#if defined(CONFIG_BCM_TCH_BL) && defined(CONFIG_BCM963268)
        /* Restore RESERVED bit to the Power On Reset default of Broadcom to fix NG-93203 */
        extsw_rreg_wrap(PAGE_CONTROL, REG_SWITCH_MODE, &v8, 1);
        v8 |= 0x04;
        extsw_wreg_wrap(PAGE_CONTROL, REG_SWITCH_MODE, &v8, 1);
#endif
   ...
   if (UtilGetChipIsLP())
   {
       ...
       e.port = 8;
       e.unit = 1;
       bcmeapi_ioctl_extsw_port_erc_config(&e);
   }
#endif
}
#endif
...
void bcmeapi_reset_port_mib_ext(int port)
{
    ...
    if (pVnetDev0_g->extSwitch->present) {
        ...
        extsw_rreg_wrap(PAGE_MANAGEMENT, REG_RST_MIB_CNT_EN, &rst_mib_en, 4);
        rst_mib_en = (rst_mib_en & ~REG_RST_MIB_CNT_EN_PORT_M) | 1 << port;
        extsw_wreg_wrap(PAGE_MANAGEMENT, REG_RST_MIB_CNT_EN, &rst_mib_en, 4);
        ...
        rst_mib_en |= REG_RST_MIB_CNT_EN_PORT_M;
        extsw_wreg_wrap(PAGE_MANAGEMENT, REG_RST_MIB_CNT_EN, &rst_mib_en, 4);
    }
...
int bcmsw_get_hw_stats(int port, int extswitch, struct rtnl_link_stats64 *stats)
{
    ...
    if (extswitch) {
        ...
        extsw_rreg_wrap(PAGE_CONTROL, REG_LOW_POWER_EXP1, data, 4);
        if (*(uint32 *)data & (1 << port)) {
            spin_unlock_bh(&sf2_stat_access);
            printk("Err: port=%d is in low power mode - MIB counters not accesible!!\n", port);
            return -1;
        }
        extsw_rreg_wrap(PAGE_CONTROL, REG_SW_RESET, data, 4);
        if (*(uint32 *)data &  REG_SW_RST) {
            spin_unlock_bh(&sf2_stat_access);
...
static int sf2_bcmsw_dump_mib_ext(int port, int type)
{
    ...
    if (type)
    {
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), SF2_REG_MIB_P0_TXOCTETS, data, DATA_TYPE_HOST_ENDIAN|8);
        v32 = *((uint64 *)data);
        printk("TxOctetsLo:             %10u \n", v32);
        v32 = *((uint64 *)data) >> 32;
        printk("TxOctetsHi:             %10u \n", v32);
...
//
    ...
    if (type)
    {
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), SF2_REG_MIB_P0_RXOCTETS, data, DATA_TYPE_HOST_ENDIAN|8);
        v32 = *((uint64 *)data);
        printk("RxOctetsLo:             %10u \n", v32);
        v32 = *((uint64 *)data) >> 32;
        printk("RxOctetsHi:             %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), SF2_REG_MIB_P0_RXGOODOCT, data, DATA_TYPE_HOST_ENDIAN|8);
        v32 = *((uint64 *)data);
        printk("RxGoodOctetsLo:         %10u \n", v32);
        v32 = *((uint64 *)data) >> 32;
        printk("RxGoodOctetsHi:         %10u \n", v32);
