...
#ifdef CONFIG_TCH_KF_WLAN_PERF_RX_ENET
...
#endif /* CONFIG_TCH_KF_WLAN_PERF_RX_ENET */
/* FRV: Direct release of buffers from eth back to wl */
...
#if defined(CONFIG_BCM963268) || defined(CONFIG_BCM63381) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM94908)
...
int __init bcm63xx_enet_probe(void)
{
    ...
    spin_lock_init(&pDevCtrl->ethlock_skblist);
...
static int bcm63xx_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    switch (cmd)
    {
        ...
        case SIOCSCLEARMIBCNTR:
            ...
            if (dev->base_addr != vnet_dev[0]->base_addr)   /* not bcmsw device, operate on one port */
            {
                BcmEnet_devctrl *pDevCtrlSelf = (BcmEnet_devctrl *)netdev_priv(dev);

                for (vport = 1; vport <= vport_cnt; vport++)
                    if (vnet_dev[vport] == dev) break;
                if (vport > vport_cnt)
                    goto ErrorEfault;                       /* dev is not in vnet_dev[] */

                memset(&pDevCtrlSelf->stats, 0, sizeof(pDevCtrlSelf->stats));
                memset(&pDevCtrlSelf->cHwStats, 0, sizeof(pDevCtrlSelf->cHwStats));
                memset(&pDevCtrlSelf->lHwStats, 0, sizeof(pDevCtrlSelf->lHwStats));
                atomic_long_set(&dev->rx_dropped, 0);

                logical_port = port_id_from_dev(dev);
                port = LOGICAL_PORT_TO_PHYSICAL_PORT(logical_port);
                if (IsExternalSwitchPort(logical_port))
                    bcmeapi_reset_port_mib_ext(port);
                else
                    bcmeapi_reset_port_mib(port);
        ...
        case SIOCETHCTLOPS:
            ...
            switch(ethctl->op) {
                ...
                case ETHSETSPOWERDOWN:
                    ...
                    ethsw_switch_manage_port_power_mode(swPort, ethctl->op==ETHSETSPOWERUP);
                    val = 0;
                    break;
        ...
        case BREVT_STP_STATE_CHANGED:
            {
                ...
                if ( bcm63xx_enet_getPortFromName(&pInfo->portName[0], &unit, &port ) < 0 )
                {
                    ...
                    dev = dev_get_by_name(&init_net, pInfo->portName);
                    if (NULL == dev)
                    {
                        break;
                    }
                ...
                {
                    portMap = (1<< PHYSICAL_PORT_TO_LOGICAL_PORT(port,unit));
                }
                ...
                logical_port = 0;
                while(portMap) {
                    if (portMap & (1<<logical_port)) {
                        unit = LOGICAL_PORT_TO_UNIT_NUMBER(logical_port);
                        port = LOGICAL_PORT_TO_PHYSICAL_PORT(logical_port);
                        if ( 0 == (pDevCtrl->stpDisabledPortMap & logical_port) ) {
                            bcmeapi_ethsw_set_stp_mode(unit, port, stpVal);
                        }
                        portMap &= ~(1<<logical_port);
                    }
                    logical_port++;
                }
                break;
